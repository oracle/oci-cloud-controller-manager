...
| `oci.oraclecloud.com/oci-load-balancer-backendset-ca-bundle`                 | Specifies an OCI Certificates Service CA Bundle OCID to populate the trusted certificate authorities on the backendset SSL configuration of the LB managed by CCM. Applies only to the backend-set (not the listener) and does not replace the `service.beta.kubernetes.io/oci-load-balancer-tls-backendset-secret` mechanism.  | `N/A`                                            |                        `"ocid1.cabundle.oc1..aaa"`                        |
| `oci.oraclecloud.com/oci-load-balancer-verify-peer-certificate`              | Enables backend peer certificate verification against the trusted CA bundle on the backendset SSL configuration. Accepted truthy values: `true`, `yes`, `1`.                                                                                                                     | `false`                                          |                                         `"true"`                                         |
| `oci.oraclecloud.com/oci-load-balancer-name`                                 | Sets a custom display-name prefix for the OCI load balancer. The Service UID is appended to keep the name unique (`<name>-<uid>`), and the result is truncated to the 1024-character limit.                                                                                      | `N/A`                                            |                                      `"my-lb"`                                       |
