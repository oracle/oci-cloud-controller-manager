...
// By default response will return a body of []byte. Otherwise, if requestDetails.returnRespBodyAsStream
// is set to true, bodyAsStream will be returned with an io.ReadCloser coming from resp.body. If body
// is set, bodyAsStream will be nil and vice versa.
...
func (r *response) unmarshal(resource interface{}) (e error) {
	...
	if cs, ok := resource.(BodyUnmarshallable); ok {
		...
		if r.bodyAsStream != nil {
			if e = cs.SetBodyAsStream(r.bodyAsStream, val); e != nil {
				return
			}
		}
