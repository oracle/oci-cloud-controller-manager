...
func (s *IdentityGroupTestSuite) TestGroup() {
	client := getClient("fixtures/identity/group")
	defer client.Stop()
	id, err := resourceApply(createGroup(client))
...
func (s *IdentityGroupTestSuite) TestGroupUpdates() {
	client := getClient("fixtures/identity/updategroup")
	defer client.Stop()
	id, err := resourceApply(createGroup(client))
	s.Require().NoError(err)
	defer func() {
...
func (s *IdentityGroupTestSuite) TestListGroups() {
	client := getClient("fixtures/identity/listgroups")
	defer client.Stop()
	var gids []string
	defer func() {
		for _, id := range gids {
